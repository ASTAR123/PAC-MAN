...
FONTPATH = os.path.join(os.getcwd(), 'resources/font/ALGER.TTF')  # The True Type Font file (quoted from online)
...
def startLevelGame(level, screen, font):
    ...
    while True:
        ...
        for ghost in ghost_sprites:
            # Random track for the ghost sprites (There are bugs in execution so that we didn't use it) (We can't solve the bug)
            '''
			...
			while not res:
				...
				res = ghost.update(wall_sprites, None)
			'''
            # define the track of ghost sprites!!!
...
class Level1():
    ...
    # create the "foods" for the game
    def setupFood(self, food_color, bg_color):
        self.food_sprites = pygame.sprite.Group()
        for row in range(19):
            for col in range(19):
                if (7 <= row <= 8) and (8 <= col <= 10):
                    continue
                else:
...
'''
Function: Defining Sprites for the game
'''


'''it's the walls that you cannot step over'''
...
'''the foods which you want to eat and swallow and the most important ,to excrete lol'''
...
'''the roles that you play and just remind that this is just a game not your life'''
class Player(pygame.sprite.Sprite):
	...
	'''change the speed as well as the direction'''
	...
	'''change the location of the role you play'''
